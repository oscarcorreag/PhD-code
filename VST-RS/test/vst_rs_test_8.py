...
if __name__ == '__main__':
    ...
    np.random.seed(0)
    terminals = np.random.choice(a=m * n, size=150, replace=False)
    pois_1 = terminals[:15]
    terminals_1 = terminals[15:]
    ...
    plans, cost, warl, mwrl, mrl1, mrl2, entropy = vst_rs.non_congestion_aware(queries, 4, 8, bpr, parallelise=True)
    ...
    #     title_1="Non-congestion-aware",
    #     title_2="Cost (w. congestion): " + str(round(cost, 2)) + ", Avg. Congestion: " + str(round(avg_congestion, 2)) +
